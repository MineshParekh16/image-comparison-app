  ...
  for (const file of files) {
    ...
    if (file.match(/\.(jpg|jpeg|png)$/i)) {
      try {
        ...
        if (!exists) {
          const croppedHashes = await getCroppedHashes(filePath);
          await new Image({
            imageHash: hash,
            imageUrl: filePath,
            croppedHashes,
...
app.post('/compare', upload.single('image'), async (req, res) => {
  try {
    ...
    // Partial/cropped match logic (patch vs patch)
    for (const chash of croppedHashes) {
      for (const image of images) {
        if (!image.croppedHashes) continue;

        for (const storedHash of image.croppedHashes) {
          const distance = hammingDistance(chash, storedHash);
          const similarity = ((64 - distance) / 64) * 100;
          ...
          if (similarity >= 70) {
            partialMatches.push({
              imageUrl: image.imageUrl,
              similarity: similarity.toFixed(2),
              note: 'Design/partial region match (patch-to-patch)',
            });
            break;
          }
        }
    ...
    if (partialMatches.length > 0) {
      return res.status(200).json({
        message: 'Partial or design-based matches found',
        data: partialMatches,
      });
    }
